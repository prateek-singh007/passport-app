...
def reduce_size(image, parameter, option):
    ...
    '''ik = imutils.resize(image, height=size[0])
    ik2= cv2.resize(image,size)
    cv2.imshow('xxx',(image_resize(image,parameter[0],parameter[1])))
    cv2.waitKey(0)
    cv2.imshow('image0000000', ik)
    cv2.imshow('image1111111', ik2)
    cv2.waitKey(0)'''
